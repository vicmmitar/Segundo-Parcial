...
namespace WindowsFormsApp2
{
    public partial class Form1 : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            data.Fill(tabla);
            con.Close();
            //Inicializacion de las imagenes nomas
            Bitmap bmp2 = new Bitmap(pictureBox2.Image);
            Bitmap cpoa2 = new Bitmap(bmp2.Width, bmp2.Height);
            Dictionary<string, int> colorChangeCount = new Dictionary<string, int>();

            //procesamiento de 10 por 10 pixeles
            for (int i = 0; i < bmp2.Width; i += 10)
            {
                for (int j = 0; j < bmp2.Height; j += 10)
                {
                    ...
                    int avgG = totalG / 100;
                    int avgB = totalB / 100;
                    //comparacion con la base de datos y cambio de color
                    ...
                    for (int row = 0; row < tabla.Rows.Count; row++)
                    {
                            ...
                            int.TryParse(tabla.Rows[row]["B"].ToString(), out dbB))
                        {
                            ...
                            {
                                string colorName = tabla.Rows[row]["color"].ToString();
                                ...
                                string message = $"""Textura {textura} con RGB ({dbR}, {dbG}, {dbB}) cambiada a color {colorName}""";

                                if (colorChangeCount.ContainsKey(message))
                                {
                                    colorChangeCount[message] += pixelsChanged;
                                }
                                else
                                {
                                    colorChangeCount[message] = pixelsChanged;
            ...
            if (colorChangeCount.Count > 0)
            {
                StringBuilder resultMessage = new StringBuilder();
                foreach (var entry in colorChangeCount)
                {
                    resultMessage.AppendLine($"{entry.Key} con {entry.Value} píxeles cambiados");
                }
                MessageBox.Show(resultMessage.ToString(), "Texturas y colores cambiados");
            }
            else
            {
